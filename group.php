<?php
//on filtre pour empêcher qu'on ne récup pas des saletés 
...
if (empty($get['id'])&& empty($get['name'])) {
    ...
    $stmt = $pdo->prepare('SELECT `album`.*, `group`.name AS group_name  FROM `album` INNER JOIN `group` ON `album`.group_id=`group`.id WHERE group_id=:id'); //groupe_name est le nom renommé pour ne pas créer de conflit le AS permet de renommer
    //3.On remplace les valeurs dans la requête.
    $stmt->bindValue(':id', $get['id']);
    //4. On envoie la requête à MariaDB. 
    $stmt->execute();
    //5. On récupére les données.
    $albums = $stmt->fetchAll(PDO::FETCH_ASSOC); //stmt = statement = objet renvoyé par le PDO sur un stmt on execute juste notre requête
} catch (Exception $e) {
    header('Content-Type: text/plain');
